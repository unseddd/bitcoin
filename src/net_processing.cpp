                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::SKETCH) {
        LOCK(peer->m_recon_state_mutex);
        if (peer->m_recon_state == nullptr) return;
        if (peer->m_recon_state->m_outgoing_recon != RECON_INIT_REQUESTED) return;
        ...
        if (remote_sketch_capacity == 0 || !remote_sketch || !local_sketch) {
            ...
            std::vector<uint256> remote_missing(peer->m_recon_state->m_local_set.begin(), peer->m_recon_state->m_local_set.end());
