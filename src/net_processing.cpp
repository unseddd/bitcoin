...
size_t PeerManagerImpl::GetFloodingOutboundsCount(const CNode& skip_node) const
{
    ...
    m_connman.ForEachNode([this, &result, &skip_node](const CNode* pnode) {
        ...
        if (peer->m_recon_state) {
            ...
            if (peer->m_recon_state->m_flood_to) ++result;
        } else {
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::SKETCH) {
        LOCK(peer->m_recon_state_mutex);
        if (peer->m_recon_state == nullptr) return;
        if (peer->m_recon_state->m_outgoing_recon != RECON_INIT_REQUESTED &&
            peer->m_recon_state->m_outgoing_recon != RECON_EXT_REQUESTED) return;
        ...
        if (peer->m_recon_state->m_outgoing_recon == RECON_EXT_REQUESTED) {
    ...
    if (msg_type == NetMsgType::REQSKETCHEXT) {
        LOCK(peer->m_recon_state_mutex);
        if (peer->m_recon_state == nullptr) return;
        if (peer->m_recon_state->m_incoming_recon != RECON_INIT_RESPONDED) return;
    ...
    if (msg_type == NetMsgType::RECONCILDIFF) {
        LOCK(peer->m_recon_state_mutex);
        if (peer->m_recon_state == nullptr) return;
        if (peer->m_recon_state->m_incoming_recon != RECON_INIT_RESPONDED &&
            peer->m_recon_state->m_incoming_recon != RECON_EXT_RESPONDED) return;
        bool recon_result;
